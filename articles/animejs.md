...
1.  **Animations** –
    ...
    accumulate a multi-segment animation specification in R.
2.  **Properties** –
...
Any SVG whose elements can be identified by a valid CSS selector can be
targeted directly.
...
[`anime_animate()`](https://long39ng.github.io/animejs/reference/anime_animate.md)
is the R equivalent of Anime.js v4’s `animate(targets, parameters)`: one
set of property animations applied to one selector, without a timeline.
...
Single animations support the same playback modifiers, event callbacks,
and rendering options as timelines.
...
with an optional CSS unit and per-property easing override.
...
[`anime_keyframes()`](https://long39ng.github.io/animejs/reference/anime_keyframes.md)
accepts two or more positional values. Bare values are used as `to`
values; lists may additionally specify `ease`, `duration`, and `delay`
per keyframe.
...
All easing constructors return `anime_easing` objects that map onto
their Anime.js v4 equivalents:
...
| [`anime_easing_back()`](https://long39ng.github.io/animejs/reference/anime_easing.md) | `"outBack(1.70158)"` |
| [`anime_easing_spring()`](https://long39ng.github.io/animejs/reference/anime_easing.md) | `spring({bounce: .5, duration: 628})` |
...
for cubic bezier, steps, and spring easings – the R constructors handle
this automatically by reconstructing the corresponding function calls in
JavaScript.

## Playback

[`anime_playback()`](https://long39ng.github.io/animejs/reference/anime_playback.md)
sets looping, direction, speed, and UI controls on a timeline or single
animation. Arguments left as `NULL` keep the settings already stored on
the object.
...
registers a global JavaScript function as a lifecycle callback. Inject
the function into the page via `htmltools::tags$script()`.
...
Valid events are `"onBegin"`, `"onBeforeUpdate"`, `"onUpdate"`,
`"onRender"`, `"onLoop"`, `"onPause"`, and `"onComplete"`, matching the
Anime.js v4 callback API.
